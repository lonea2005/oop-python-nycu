    ...
    def player_turn(self):
        ...
        if self.action==1:
            ...
            if self.remain_bullet[0]&self.handsaw:
                ...
                self.player.hp -= 1
                pretext ='你射中了自己,造成一點傷害'
                self.computer.pop_bullet_pattern()
                self.live_bullet -= 1
                current_turn = 'dealer'
            else:
                pretext ='你的子彈打空了,額外獲得一回合'
                self.computer.pop_bullet_pattern()
                self.blank -= 1
                #continue
            self.remain_bullet.pop(0)
            ...
            if self.choose == '手鋸':
                if self.handsaw:
                    ...
                    return
                self.handsaw = True
                pretext ='你使用了手鋸,下一發子彈造成兩倍傷害'
            elif self.choose == '啤酒':
                if self.remain_bullet.pop(0):
                    pretext ='你使用了啤酒,退掉一發實彈'
                    self.live_bullet -= 1
                else:
                    pretext ='你使用了啤酒,退掉一發空包彈'
                    self.blank -= 1
                self.computer.pop_bullet_pattern()
                ...
                if random.randint(0,1):
                    pretext ='你使用了過期藥物,失去了一點血量'
                    self.player.hp -= 1
                else:
                    pretext ='你使用了過期藥物,回復了2點血量'
                    self.player.hp += 2
                if self.player.hp <= 0:
                    time.sleep(2)
                    print('**************************************')
                ...
                pretext ='你使用了放大鏡看到 ','實彈' if self.remain_bullet[0] else '空包彈'
            elif self.choose == '香菸':
                pretext ='你使用了香菸,回復一點血量'
                self.player.hp += 1
            elif self.choose == '手銬':
                if self.computer.handcuff:
                    pretext ='莊家已經被銬住了'
                    return
                pretext ='你使用了手銬,莊家下回合無法行動'
                ...
                if self.handsaw:
                    self.computer.hp -= 6
                    pretext ='你發射了大口徑子彈,造成6點傷害'
                else:
                    self.computer.hp -= 3
                    pretext ='你發射了大口徑子彈,造成3點傷害'
                if self.remain_bullet.pop(0):
                    self.live_bullet -= 1
                else:
                    self.blank -= 1
                ...
                if self.remain_bullet.pop(0):
                    if self.computer.fog > 0:
                        ...
                        pretext ='你發射了榴彈砲,造成',2*damage,'點傷害'
                        self.handsaw = False
                    else:
                        self.computer.hp -= damage
                        pretext ='你發射了榴彈砲,造成',damage,'點傷害'
                    self.live_bullet -= 1
                else:
                    pretext ='你發射了榴彈砲,但是子彈打空了'
                    self.blank -= 1
                self.computer.pop_bullet_pattern()
                current_turn = 'dealer'
                ...
                if self.handsaw:
                    damage *= 2
                pretext ='你使用了彈藥包,對莊家造成',damage,'點傷害'
                self.computer.hp -= damage
                self.handsaw = False
                self.remain_bullet = []
                self.live_bullet = 0
...
# Fonts
try:
    font = pygame.font.Font('NotoSansTC\NotoSansTC-VariableFont_wght.ttf', 24)
    description_font = pygame.font.Font('NotoSansTC\NotoSansTC-VariableFont_wght.ttf', 18)
...
def enter_shop():
    ...
    pygame.display.flip()
    npc_interaction("Leviathan", "normal", draw_shop)
...
item_buttons = [
    ...
    Button("禁藥", 20, 20, 40, 40, use_drug, "70%機率血量大幅提升，30%機率中毒\n     由3個過期藥物合成"),
    Button("大口徑子彈", 20, 20, 40, 40, use_big, "當前子彈換成大口徑子彈並直接射出，造成3點傷害，注意這會結束你的回合\n     由2個手鉅和1個放大鏡合成"),
    Button("榴彈砲", 20, 20, 40, 40, use_bomb, "使用榴彈砲攻擊，若為實彈則造成自身當前血量的傷害，注意這會結束你的回合並使你剩下1點血量\n     由1個大口徑子彈、1個腎上腺素和1個轉換器合成"),
    Button("彈藥包", 20, 20, 40, 40, use_bullet, "直接射出所有子彈並重新裝填八顆子彈\n     由2個啤酒和1個香菸合成"),
    #Button("擴增背包", 20, 20, 40, 40, use_pack, "背包空間+2\n     由滿背包任意的物品合成"),
    Button("琉璃皇后", 20, 20, 40, 40, use_glass_queen, "獲得琉璃的祝福，退出所有子彈並重新裝填一發實彈\n接下來每次換彈都會回復1點血量，並且根據第一發子彈獲得不同增益效果"),
